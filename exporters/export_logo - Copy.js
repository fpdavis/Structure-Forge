(function(){
  ...
  // - We transform app coordinates into LOGO coords, fix rotation, flip Y, and fit to ~1000x800
  ...
  function c255to99(v){
    ...
    return Math.max(0, Math.min(99, Math.round(n * 99 / 255)));
  }
  ...
  // Return a LOGO color token:
  // - rgba(...) -> [r g b a] with r/g/b converted to 0..99
  // - rgb(...)  -> [r g b]   with r/g/b converted to 0..99
  // - #rrggbb   -> [r g b]   converted to 0..99
  ...
  function logoColorToken(css){
    ...
    if(mRGBA){
      const r = c255to99(mRGBA[1]), g = c255to99(mRGBA[2]), b = c255to99(mRGBA[3]);
      const a = Number(mRGBA[4]);
      return `[${r} ${g} ${b} ${isFinite(a)?a:1}]`;
    }
    const mRGB = t.match(/^rgb\(\s*([0-9.]+)\s*,\s*([0-9.]+)\s*,\s*([0-9.]+)\s*\)$/i);
    if(mRGB){
      const r = c255to99(mRGB[1]), g = c255to99(mRGB[2]), b = c255to99(mRGB[3]);
      return `[${r} ${g} ${b}]`;
    }
    ...
    // named CSS colors: use word form
    return `"${t}`;
  }
  ...
  function normalizeRectRelToRoomPrimary(item, room){
    ...
    // Convert item anchor into "room primary" coordinates (room primary = room's own corner setting)
    if(room.corner==="NE"){ x = room.wIn - x; }
    else if(room.corner==="SW"){ y = room.hIn - y; }
    else if(room.corner==="SE"){ x = room.wIn - x; y = room.hIn - y; }

    // Now convert item corner -> NW
  ...
  registerExporter({
    ...
    export(ctx, opts){
      ...
      // Collect rects to compute bbox (in app inches)
      const rects = [];
      ...
      for(const f of floors){
        for(const room of (f.rooms||[])){
          if(visType("Room")){
            ...
            rects.push({kind:"room", floor:f, room, rect:{xIn:rr.xIn, yIn:rr.yIn + yOffsetIn, wIn:rr.wIn, hIn:rr.hIn}});
          }
          for(const it of (room.items||[])){
            if(!visType(it.type)) continue;
            const ir = itemAbsRect(it, room);
            rects.push({kind:"item", floor:f, room, item:it, rect:{xIn:ir.xIn, yIn:ir.yIn + yOffsetIn, wIn:ir.wIn, hIn:ir.hIn}});
          }
        }
      ...
      // Fit to drawing area ~1000x800 (LOGO coords are arbitrary units; we treat them as "px-like")
      const targetW = 1000;
      const targetH = 800;
      const margin  = 50;
      ...
      // - rotate 90° clockwise to compensate the observed 90° CCW rotation in interpreters
      ...
      out.push("; Coordinates are transformed to fit ~1000x800.");
      out.push("CLEARSCREEN");
      out.push("PU");
      out.push("SETHEADING 0");
      out.push("SETPENSIZE 3");
      out.push(`SETPENCOLOR ${logoColorToken("#0000FF")}`);
      ...
      function drawRoomLabels(room, rApp){
        ...
        const dims = `${room.wIn?fmtNum(room.wIn):fmtNum(rApp.wIn)}x${room.hIn?fmtNum(room.hIn):fmtNum(rApp.hIn)} in`;

        // Put name above center, dims below center in LOGO space.
        const namePt = {x:cLogo.x, y:cLogo.y + 14};
        const dimsPt = {x:cLogo.x, y:cLogo.y - 14};

        out.push("; room labels");
        out.push("PU");
        out.push("SETLABELHEIGHT 14");
        moveTo(namePt);
        out.push(`LABEL ${labelBracket(name)}`);
        moveTo(dimsPt);
        out.push(`LABEL ${labelBracket(dims)}`);
      }
      ...
      for(const r of rects){
        if(r.kind==="room"){
          ...
          // outline always blue per requirement; fill uses item color if provided
