...
function _hydrateStructure(s){
  ...
  for(const floor of (s.house?.floors||[])){
    for(const room of (floor.rooms||[])){
      ...
      room.zIndex = Number.isFinite(Number(room.zIndex)) ? Math.trunc(Number(room.zIndex)) : getDefaultZIndexForType("Room", s);
      for(const item of (room.items||[])){
        ...
        item.zIndex = Number.isFinite(Number(item.zIndex)) ? Math.trunc(Number(item.zIndex)) : getDefaultZIndexForType(item.type, s);
    ...
    for(const s of app.structures||[]){
      ...
      for(const f of s.house?.floors||[]){
        for(const r of f.rooms||[]){
          ...
          r.locked=!!r.locked;
          r.zIndex=Number.isFinite(Number(r.zIndex)) ? Math.trunc(Number(r.zIndex)) : getDefaultZIndexForType("Room", s);
          for(const it of r.items||[]){
            ...
            it.locked=!!it.locked;
            it.zIndex=Number.isFinite(Number(it.zIndex)) ? Math.trunc(Number(it.zIndex)) : getDefaultZIndexForType(it.type, s);
          }
        }
      }
    }
